...
import { Receipt, AlertTriangle, CheckCircle2, Loader2, ScanLine } from "lucide-react";
...
export function BillSummary() {
  ...
  const { session: piSession, items: piItems, unscannedCount } = usePiCartStore();
        ...
        <AnimatePresence mode="wait">
          {piFlagged > 0 ? (
            ...
            >
              ...
              Remove {piFlagged} flagged item{piFlagged > 1 ? "s" : ""} to checkout
            </motion.div>
          ) : piPending > 0 ? (
            <motion.div
              key="pending"
            ...
            >
              ...
              <span className="relative z-10">
                {piItems.length === 0 ? "Cart Empty" : "Checkout →"}
              </span>
            </motion.button>
          )}
        </AnimatePresence>
  ...
  // ── Demo / web mode: items come from the local cartStore ─────────────────
